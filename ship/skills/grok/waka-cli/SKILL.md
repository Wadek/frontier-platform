...
Canonical local coding CLI. Source lives with your local coding-agent tree (often `./ai_learning/waka-cli`).
Command on PATH: **`waka-cli`** only. Do not install or use `waka` or `waka-bot`
as CLI aliases — those names caused confusion with separate chat/agent trees.
...
8. Advanced planning: `waka-cli plan` (cloud after egress). Atomic edits: local Ollama via `waka-cli -p`. If the local coder errors or writes no files, `-p` may fall back to a cloud codegen path on the same prompt.
9. Session `/model` choices: a local coder tag, `deepseek-flash` (cloud), `deepseek-v4-pro` (cloud).

## Not this product

- Older agent/chat consolidation trees whose `cli/` is stale — point runners at the current waka-cli source.
- Web chat UIs named waka-bot / waka-chat — not the coding CLI.
- Ollama alias tags — model tags only, not shell commands.
