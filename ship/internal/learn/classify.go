...
func renderMarkdown(ls *Landscape) string {
	...
	b.WriteString("\n## Next\n\n")
	b.WriteString("- `frontier ready` to see first-deploy gaps (CI, tests, deploy script)\n")
...
func inferKind(ls *Landscape) (kind, conf string, reasons []string) {
	...
	case containsEntry(ls.TopEntries, "gateway/") || containsEntry(ls.TopEntries, "vigil/"):
		reasons = []string{"network edge indicators (gateway/vigil)"}
		if ls.HasCompose {
...
func walkLight(root string) (langs map[string]int, manifests []string, files int) {
	...
	_ = filepath.Walk(root, func(path string, info os.FileInfo, err error) error {
		...
		if info.IsDir() {
			...
			// skip very deep trees (model weights, media)
