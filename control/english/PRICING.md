...
Vision: **flash ✓**, v4-pro ✗ — all image work uses flash.

## Platform rules

1. Pro provider never at peak (`control workflow review` → `deferred`; job service schedules off-peak).
2. Daily one-shot check of the pricing page (OS scheduler; alerts on change — DeepSeek
   reserves the right to adjust prices and says to check regularly).
3. Local first: the on-host model runs before any cloud provider.
4. All cloud calls egress-filtered first; refused trees never cross.
5. Spend ledgers per project (tokens + USD), fed into the data index.

Config: `control/pricing.yaml` (window + rates, editable without code).
