-- Tower.Weather — the pricing policy as truth (F5 witness).
...
route kind now
  | kind `elem` localKinds = Decision "local" "qwen2.5-coder:64k" False "local-first"
  ...
  | otherwise = Decision "local" "qwen2.5-coder:64k" False "default local"
